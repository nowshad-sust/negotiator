  ...
  <v-card v-if="!error" class="mx-auto mt-5 mb-3" max-width="300">
    ...
    <v-list-item>
      ...
      <v-list-item-subtitle
        >{{ windSpeed }} {{ windUnit }}</v-list-item-subtitle
      >
    </v-list-item>
  </v-card>
...
export default class Weather extends Vue {
  private feelsLikeText = constants.feelsLike;
  private windUnit = constants.windUnit;
  name = 'London';
  temp = 0;
  feelsLike = 0;
  windSpeed = 0;
  ...
  async fetchWeather() {
    ...
    try {
      ...
      actions.setError(constants.weatherError);
    }
  }
}
</script>
